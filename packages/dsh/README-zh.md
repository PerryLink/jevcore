...
模型 id 必须是 System One 的 id：裸的 `jev-*`，或者 `typesafe/jev-*`。
任何其他 id 都会被路由到聊天模型，它给出的散文式回答这个插件无法解读为决策，
因此它会在调用之前被拒绝，而不是在调用之后被误读。
...
**未核实，或已知损坏**
- **TypeSafe 路由从未针对真实 API 实际跑过。** 当时没有可用的 TypeSafe
  凭据，因此 `LiveProvider` 是只针对注入的桩和厂商的类型定义做的覆盖 ——
  比一次真实调用更弱。两条路由使用相同的原语，因此一个 TypeSafe key
  *预期* 无需改动即可工作；那是一个预期，不是一个观察结果。
- **在当前正在运行的那个构建上，`jev_ask` 的描述里有一个损坏的破折号。** 它
  ...
  在修复之前就加载了它的模块，不重启就无法重新读取。仅是外观问题；它不改变
  任何行为。
