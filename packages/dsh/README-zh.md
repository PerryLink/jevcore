...
TypeSafe 相同的 `POST /v1/systemone` 路径上托管 System One 模型，就在它自己
API 根的下一层，因此这是通往 Jev 的有文档记载的路由，而不是一种近似：
...
### 在没有谁能作答的地方启用 safety 闸门

闸门给出的回答是 `allow`、`deny` 或 `ask`，而 `ask` 既是对一个被触发的风险的
回答，也是对一个无法判定的裁决的回答。一个 `ask` 需要审批通道：没有组合任何
审批服务的部署无法把问题升级给人类，harness 会把这次询问解析为拒绝，而不是
执行调用。闸门按工具名片段匹配，它的清单包括 `pwsh`、`bash`、`git`、`write`
和 `edit`，因此在这样的部署里启用它的人，看到的是普通 shell 命令和文件编辑
接连失败。请在一个能把问题交给人类的通道存在的地方启用它；这个通道由什么
提供，读 [docs/approval.md](docs/approval.md)。
...
- **`jev_rank`** —— 依据一个判据对候选打分并排序，每个候选一个问题，
  ...
  列表是有边界的：每个候选都会向一个固定的 4,000 字符预算里添一个问题，
  因此在默认判据下能放下 20 个候选，在 100 字符的判据下能放下 17 个，
  更长的列表会被拒绝并报错，而不是被裁剪到放得下的那些。
...
- **OpenRouter 路由已针对真实 API 核实。** `pnpm --filter jevcore run
  probe:live` 和 `pnpm --filter jevcore-mcp run smoke:live` 都针对真实的 System One
  模型作了答。记录下来的结果见根 README 的现状部分。
