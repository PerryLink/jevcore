...
`jev_rank` 接受的列表是有边界的，而不是从任意列表里挑出最好的。每个候选都会向
问题映射上那个固定的 4,000 字符预算里添一个问题，放不下的列表会被拒绝并报错，
而不是截断到放得下的前 N 个：默认判据下能放下 20 个候选，100 字符的判据下是 17 个，
500 字符的判据下是 6 个。把候选列表保持短小，把判据写得简洁，
并把长列表拆成几批。
