...
const main = async (): Promise<void> => {
  ...
  note(`[dsh-jev-mcp] ready · provider=${runtime.config.provider} · model=${runtime.config.model}`)
}

main().catch((error: unknown) => {
  note(`[dsh-jev-mcp] failed to start: ${error instanceof Error ? error.message : String(error)}`)
  process.exitCode = 1
})
