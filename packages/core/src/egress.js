...
export class EgressDeniedError extends Error {
    ...
    constructor(feature) {
        super(`egress for "${feature}" is not enabled. This feature would send content to a third ` +
...
export class EgressContract {
    ...
    assert(feature) {
        if (!this.allows(feature))
            throw new EgressDeniedError(feature);
    }
    ...
    reportLines() {
        ...
        if (!this.settings.transmitting) {
            lines.push(`[dsh-jev] provider=mock  endpoint=none  egress=OFF  ` +
            ...
            for (const line of this.lines()) {
                if (line.enabled) {
                    lines.push(`[dsh-jev]   armed  ${line.feature}  ` +
        ...
        lines.push(`[dsh-jev]   redaction is best-effort; it removes named fields and known secret ` +
 ...
 */
const capJsonText = (text, maxChars) => {
    if (text.length <= maxChars) {
        return { text, value: JSON.parse(text), truncated: false };
    }
    const value = {
        '[truncated]': true,
